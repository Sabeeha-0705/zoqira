...
// helper to send token and user (without password)
const sendTokenResponse = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    const generateRefreshToken = (userId) => {
      ...
      return jwt.sign({ id: userId }, secret, { expiresIn: process.env.REFRESH_EXPIRES || '30d' });
    };
    ...
    exports.register = async (req, res, next) => {
      try {
        ...
        return res.status(201).json({ success: true, token: accessToken, refreshToken, user: buildSafeUser(user) });
      } catch (error) {
        return next(error);
      }
    };
    ...
    exports.login = async (req, res, next) => {
      try {
        ...
        return res.status(200).json({ success: true, token: accessToken, refreshToken, user: buildSafeUser(user) });
      } catch (error) {
        return next(error);
      }
    };
    ...
    exports.refresh = async (req, res, next) => {
      try {
        ...
        return res.status(200).json({ success: true, token: newAccess, refreshToken: newRefresh });
      } catch (error) {
        return next(error);
      }
    };
    ...
    exports.logout = async (req, res, next) => {
      try {
        ...
        if (res.cookie) res.cookie('token', 'none', { expires: new Date(Date.now() + 10 * 1000), httpOnly: true });
